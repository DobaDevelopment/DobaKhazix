...
function Khazix.LoadMenu()
    ...
    Menu.NewTree("KS", "Kill Secured Settings", function()
        ...
        Menu.Checkbox("KS.E"," Use E", true) end)
...
local function dmg(spell)
    ...
    if spell.Key == "E" then
        dmg =  (65 + (spell:GetLevel() - 1) * 35) + (0.2 * Player.BonusAD) end return dmg end
...
function Khazix.Auto()
    if KS(Q) then
        for k,v in pairs(GetTargets(Q)) do
            if v then
                ...
                if dmg > Ks and Q:Cast(v) then return end end end end
    if KS(W) then
    ...
    if KS(E) then
        for k,v in pairs(GetTargets(E)) do
            ...
            if dmg > Ks and E:CastOnHitChance(v,Enums.HitChance.High) then return end end end end
end
